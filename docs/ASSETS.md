...
| `modules/04-classical-ml/data/bank-marketing-sample.csv` | M4 — capstone ("will this customer subscribe?", naturally imbalanced 11.7% yes; the `duration` column kept deliberately as the leakage landmine) | UCI ML Repository ID 222 (Moro, Laureano & Cortez 2011); seeded stratified 1,500-row sample by `infra/datasets/make_bank_sample.py` (seed 0; vendored 2026-07-27) | CC BY 4.0 (UCI dataset page; citation required and given in the capstone) | 1,500 (of 45,211) | ~120 KB — documented exception to the ≤~50 KB rule (DECISIONS D0021) |
...
| UCI Bank Marketing | M4 capstone | **vendored** — see the sample row in the Datasets table above (2026-07-27) | CC BY 4.0 — Moro, Laureano & Cortez (2011), UCI ML Repository ID 222; citation required |
